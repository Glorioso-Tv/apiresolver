...
def getRequest(url,origin=False,referer=False,post=False,type='encode'):
    req = Request(url)
...
def getRequest2(url,origin=False,referer=False,post=False):
    ...
    if post:
        ...
        response = requests.get(url, data=json.dumps(post), headers=headers)
    data = response.text
    return data


def test():
    url = 'https://vc.mycujoo.tv/view'
    data = getRequest2(url,origin='https://elevensports.com',referer='https://elevensports.com/',post={"id":"ckxaq2aax31p60gblyjgh3yga","type":"event"})
    print(data)
